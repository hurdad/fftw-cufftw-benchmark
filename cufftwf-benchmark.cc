...
static void cu_fftwf(benchmark::State& state) {
	int N = state.range(0);
	for (auto _ : state) {
		...
		fftwf_plan plan = fftwf_plan_dft_1d(N, in, out, FFTW_FORWARD, FFTW_FORWARD);
		fftwf_execute(plan);
...
BENCHMARK(cu_fftwf)->RangeMultiplier(2)->Range(1<<10, 1<<26)->Complexity();
BENCHMARK_MAIN();
